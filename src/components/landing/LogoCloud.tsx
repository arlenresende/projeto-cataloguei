...
export function LogoCloud() {
  return (
    <section
      className="border-y border-zinc-200 bg-zinc-50 py-12 md:py-16"
      ...
      <div className="mx-auto max-w-6xl px-4">
        ...
        <ul className="grid grid-cols-2 items-center justify-items-center gap-x-8 gap-y-6 sm:grid-cols-3 md:grid-cols-6">
          {LOGOS.map((logo) => (
            <li
              key={logo}
              className="text-base font-semibold text-zinc-500 transition-colors hover:text-zinc-700 sm:text-lg"
            >
              {logo}
            </li>
          ))}
